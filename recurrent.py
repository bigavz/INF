...
Iext = np.zeros(N) # externally applied stimulus = ARRAY
Iext[0] = 1.5
...
for i, t in enumerate(time[1:],1):
    active = np.nonzero(t > last_spike + tau_ref) #list of neurons numbers
    Vm[active,i] = Vm[active,i-1] + (-Vm[active,i-1] + I[active,i-1]) / tau_m * dt

    spiked = np.nonzero(Vm[:,i] > Vth) #spiked returns indices i.e. list of neurons
    last_spike[spiked] = t
    raster[spiked,i] = spiked[0]+1 #current timestep (column) =
    I[:,i] = Iext + synapses.dot(Isyn(t - last_spike)) #lastspike = t vector
